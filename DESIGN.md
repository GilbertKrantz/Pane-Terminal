...
Idle autocomplete waits 110 ms after the current token changes and then presents at most 12 deduplicated results from zsh's own context-sensitive completion system. Tab accepts the first visible result, Escape dismisses the current query, and clicking accepts a specific result.
